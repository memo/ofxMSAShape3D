...
void testApp::draw() {
	...
	for(int i=0; i<5000; i++) {
		...
		pos += ofVec3f(cos_t, sin_t, sin(curTime * 0.5f + i*0.002f) * 0.5);
		...
		ofVec3f pos2 = pos + ofVec3f(sin_t, -cos_t) * ofMap(sin(curTime * 0.4f * ( 1.0f + i * 0.001f) + i * 0.06f + sin(i*0.001f) * 0.2f), -1, 1, 5, 10 +  i * 0.01f);
		
		// place second vertex (can also pass pointer (array) to coordinates)
		myObj.addVertex2v(pos2.getPtr());
	}
